        ...
        <Modal isOpen={true} onClose={onClose} title="User Profile Details" maxWidthClass="max-w-2xl">
            <div className="space-y-6 max-h-[80vh] overflow-y-auto pr-2">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-6 p-6 bg-gradient-to-br from-primary-500/10 to-indigo-500/10 rounded-3xl border border-primary-500/20">
                    <img src={user.imageUrl || createAvatar(user.name)} alt={user.name} className="w-24 h-24 sm:w-32 sm:h-32 rounded-3xl object-cover ring-4 ring-white dark:ring-slate-800 shadow-2xl" />
                        ...
                        {students.map(user => (
                            ...
                            >
                                <div className="flex items-center gap-4 min-w-0 flex-1">
                                    <img className="h-12 w-12 rounded-full object-cover ring-2 ring-slate-100 dark:ring-slate-700 group-hover:ring-primary-500/30 transition-all shrink-0" src={user.imageUrl || createAvatar(user.name)} alt={user.name} />
     ...
     <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-lg">
        ...
        <div className="space-y-2">
            {users.map(user => (
                ...
                >
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                        <img className="h-12 w-12 rounded-full object-cover ring-2 ring-white dark:ring-slate-800 group-hover:ring-primary-500/30 transition-all shrink-0" src={user.imageUrl || createAvatar(user.name)} alt={user.name} />
                        <div className="min-w-0 flex-1">
                            <div className="text-sm font-bold text-slate-900 dark:text-slate-100 truncate">{user.name}</div>
                            <div className="text-xs font-mono font-bold text-slate-500 dark:text-slate-400 truncate">{user.pin}</div>
                        </div>
