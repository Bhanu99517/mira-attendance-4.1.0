      ...
      <>
          <aside className={`fixed top-0 left-0 z-40 w-64 h-screen bg-white dark:bg-slate-900 flex flex-col transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:animate-slide-in-from-left`}>
                ...
                <nav className="flex-1 px-3 py-4 space-y-6 overflow-y-auto sidebar-scroll">
                    {navLinks.map((section) => {
                        ...
                        const filteredLinks = section.links.filter(link => {
                            ...
                            // 2. Restriction for bhanu99517: Only ManageUsers and ManageColleges
                            if (isBhanuAdmin) {
                                return link.name === 'ManageUsers' || link.name === 'ManageColleges';
                            }
                ...
                <div className="p-4 border-t border-slate-200 dark:border-white/10">
                    <button onClick={() => setLogoutModalOpen(true)} className="w-full flex items-center px-4 py-3 text-sm font-bold rounded-xl text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 transition-all duration-200 group">
                        <Icons.logout className="h-5 w-5 mr-3 transition-transform group-hover:translate-x-1" />
                        <span>Logout</span>
                    </button>
                </div>
            </aside>
...
const Header: React.FC = () => {
  const { theme, toggleTheme, user, setSidebarOpen, logout } = useAppContext();
  const navigate = useNavigate();
  ...
  return (
    <header className="sticky top-0 bg-slate-100/80 dark:bg-slate-900/80 backdrop-blur-lg z-20 border-b border-slate-200 dark:border-slate-800 animate-fade-in-down">
        <div className="px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between min-h-[5rem] py-2 gap-2">
                 {!isBhanuAdmin && (
                ...
                <div className="flex items-center space-x-2 sm:space-x-4 shrink-0">
                    {isBhanuAdmin && (
                        <button 
                            ...
                            className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-xl transition-all"
                        >
                            <Icons.logout className="h-5 w-5" />
                            <span>Logout</span>
                        </button>
                    )}
                    <button onClick={toggleTheme} className="p-1.5 sm:p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors">
                        {theme === 'light' ? <Icons.moon className="h-5 w-5 sm:h-6 sm:w-6" /> : <Icons.sun className="h-5 w-5 sm:h-6 sm:w-6" />}
                    </button>

                    <div className="flex items-center space-x-2">
                        <img className="h-8 w-8 sm:h-11 sm:w-11 rounded-full object-cover ring-2 ring-offset-2 ring-offset-slate-100 dark:ring-offset-slate-900 ring-primary-500" src={user?.imageUrl} alt="User avatar" />
