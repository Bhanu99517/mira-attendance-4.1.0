...
import { User, Role, Branch, AttendanceRecord, Application, PPTContent, QuizContent, LessonPlanContent, ApplicationStatus, ApplicationType, SBTETResult, SyllabusCoverage, Timetable, Feedback, AppSettings, College } from './types';
...
let MOCK_COLLEGES: College[] = [];
if (storage.getItem<College[]>('MOCK_COLLEGES')?.length) {
    MOCK_COLLEGES = storage.getItem<College[]>('MOCK_COLLEGES')!;
} else {
    MOCK_COLLEGES = [defaultCollege];
    storage.setItem('MOCK_COLLEGES', MOCK_COLLEGES);
}
...
export const getColleges = async (): Promise<College[]> => {
    const colleges = storage.getItem<College[]>('MOCK_COLLEGES') || [];
    return delay(colleges);
};

export const addCollege = async (college: Omit<College, 'id'>): Promise<College> => {
    const colleges = storage.getItem<College[]>('MOCK_COLLEGES') || [];
    const newCollege: College = { ...college, id: `coll_${Date.now()}` };
    colleges.push(newCollege);
    storage.setItem('MOCK_COLLEGES', colleges);
    MOCK_COLLEGES = colleges;
    return delay(newCollege);
};

export const updateCollege = async (id: string, collegeData: Partial<College>): Promise<College> => {
    let colleges = storage.getItem<College[]>('MOCK_COLLEGES') || [];
    let updatedCollege: College | undefined;
    colleges = colleges.map(c => {
        if (c.id === id) {
            updatedCollege = { ...c, ...collegeData };
            return updatedCollege;
        }
        return c;
    });
    if (!updatedCollege) throw new Error("College not found");
    storage.setItem('MOCK_COLLEGES', colleges);
    MOCK_COLLEGES = colleges;
    return delay(updatedCollege);
};
...
export const deleteCollege = async (id: string): Promise<{ success: boolean }> => {
    ...
    const success = colleges.length < initialLength;
    if (success) {
        storage.setItem('MOCK_COLLEGES', colleges);
        MOCK_COLLEGES = colleges;
    }
